...
py::dict
PyFile::readTile(int dx, int dy, int part_index, int level_x, int level_y,
                 bool separate_channels)
{
    return readTiles(dx, dx, dy, dy, part_index, level_x, level_y, separate_channels);
}

py::dict
PyFile::readTiles(int dx1, int dx2, int dy1, int dy2, int part_index,
                  int level_x, int level_y, bool separate_channels)
{
    if (!_inputFile)
        throw std::runtime_error("readTiles requires a File opened from an EXR filename");

    validate_part_index(part_index, _inputFile->parts());

    if (dx1 > dx2)
        throw std::invalid_argument("dx1 must be <= dx2");
    if (dy1 > dy2)
        throw std::invalid_argument("dy1 must be <= dy2");

    const Header& header = _inputFile->header(part_index);
    if (header.hasType() && header.type() != TILEDIMAGE)
        throw std::runtime_error("readTiles is only supported for regular tiled images");

    TiledInputPart part(*_inputFile, part_index);

    if (!part.isValidLevel(level_x, level_y))
        throw std::invalid_argument("Invalid tile level");

    if (dx1 < 0 || dx2 >= part.numXTiles(level_x))
        throw std::invalid_argument("Tile x range is outside the level");
    if (dy1 < 0 || dy2 >= part.numYTiles(level_y))
        throw std::invalid_argument("Tile y range is outside the level");

    Box2i first = part.dataWindowForTile(dx1, dy1, level_x, level_y);
    Box2i last  = part.dataWindowForTile(dx2, dy2, level_x, level_y);
    Box2i dw(V2i(std::min(first.min.x, last.min.x),
                 std::min(first.min.y, last.min.y)),
             V2i(std::max(first.max.x, last.max.x),
                 std::max(first.max.y, last.max.y)));
    ...
    for (auto c = header.channels().begin(); c != header.channels().end(); c++)
    {
        ...
        int nrgba = 0;
        if (!separate_channels)
            nrgba = P.channelNameToRGBA(header.channels(), c.name(), py_channel_name, channel_name);

        auto py_channel_name_str = py::str(py_channel_name);

        if (!tile_channels.contains(py_channel_name_str))
        {
            PyChannel C;
            ...
            tile_channels[py_channel_name.c_str()] = C;
        }

        auto v = tile_channels[py_channel_name.c_str()];
        auto C = v.cast<PyChannel&>();
    ...
    {
        py::gil_scoped_release release;
        part.readTiles(dx1, dx2, dy1, dy2, level_x, level_y);
    }
...
PYBIND11_MODULE(OpenEXR, m)
{
    ...
    m.def("setGlobalThreadCount",
          &Imf::setGlobalThreadCount,
          py::arg("count"),
          "Set the number of worker threads OpenEXR uses internally to "
          "(de)compress chunks during a single read/write call. Pass 0 to "
          "do all work on the calling thread.");
    m.def("globalThreadCount",
          &Imf::globalThreadCount,
          "Return the current OpenEXR internal worker thread count.");
    //
    ...
    py::class_<PyFile>(m, "File", R"pbdoc(
        ...
        .def("readTile", &PyFile::readTile,
             ...
             py::arg("separate_channels")=false,
             R"pbdoc(
             Read one native EXR tile from a tiled image part and return a dict of Channel objects.
             ...
             part_index : int
                 The index of the part. Defaults to 0.
             level_x : int
                 X level for mipmap/ripmap tiled files. Defaults to 0.
             level_y : int
                 Y level for mipmap/ripmap tiled files. Defaults to 0.
             separate_channels : bool
                 If False (default), R/G/B/A channels are packed as RGB/RGBA.
                 If True, channels are returned separately.
             )pbdoc")
        .def("readTiles", &PyFile::readTiles,
             py::arg("dx1"),
             py::arg("dx2"),
             py::arg("dy1"),
             py::arg("dy2"),
             ...
             py::arg("separate_channels")=false,
             R"pbdoc(
             Read a rectangular range of native EXR tiles from a tiled image part.

             The returned dict has the same Channel-object shape as channels(),
             but pixels contain only the requested tile range. For example, a
             4x4 range of 256x256 tiles returns a 1024x1024 array.
             )pbdoc")
